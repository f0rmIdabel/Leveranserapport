...
def get_input(sheet, path):
    """
    Leser excelarket basert på arknavn 
    og returnerer en dataframe
    """
    df = pd.read_excel(path+'prisliste.xlsx', decimal=",",  sheet_name=sheet)
    return df

def get_customer_pricelist(path):
    """
    Returnerer dataframe med kundespesifikk prisliste
    """
    customer = get_input("Kunder", path)
...
def get_gas_price(uke, path):
    """
    Returnerer dataframe med drivstoffpris
    """

    gas = get_input("Drivstoff", path)
